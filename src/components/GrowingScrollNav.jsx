...
const GrowingScrollNav = () => {
  ...
  useEffect(() => {
    if (position > 0 && position < window.innerHeight) {
      ...
      progress.current.style.height = position + sliderHeight/2 + "px";
      const scrollPercent =
  ...
  return (
    <>
      <div 
      ref={progress}
      className="w-[3px] bg-[#959BA2] h-0 fixed left-[250px] top-0"></div>
      <div
        ref={slider}
        className="w-[20px] bg-[#959BA2] h-[20px] rounded-full fixed left-[241px] top-0"
      ></div>
      <span ref={dateLabel} className="fixed left-[270px] top-0">
        Scroll me
      </span>
    </>
  );
...
export default GrowingScrollNav;
